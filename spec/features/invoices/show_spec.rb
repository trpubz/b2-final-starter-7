...
RSpec.describe "invoices show" do
  ...
  it "shows the un-discounted invoice and the discounted revenue from bulk ordering" do
    ...
    ii = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_8.id, quantity: 1, unit_price: 6, status: 1)
    visit merchant_invoice_path(@merchant1, @invoice_1)

    expect(page).to have_content(@invoice_1.total_revenue)
    expect(page).to have_content("Total Revenue w/ Discounts: #{@invoice_1.total_discounted_revenue}")
  ...
  it "shows the link to the discount" do
    ...
    within("#the-status-#{@ii_1.id}") do
      expect(page).to have_link(nil, href: merchant_discount_path(@merchant1, @discount1))
    end
